...
class HackedJavaEnvironment {
  ...
  HackedJavaEnvironment(ProcessingEnvironment processingEnv) {
  ...
  TypeElement getEnclosingType(Element typeElement) {
  ...
  Visibility getVisibility(Element element) {
  ...
  boolean hasOuterInstance(TypeElement typeElement) {
  ...
  private MethodDescriptor createDeclaredMethodDescriptor(
    ...
    ImmutableList.Builder<MethodDescriptor.ParameterDescriptor> parameterDescriptorBuilder =
  ...
  ImmutableList<TypeDescriptor> createTypeDescriptors(
  ...
  <T extends TypeDescriptor> ImmutableList<T> createTypeDescriptors(
  ...
  TypeDeclaration createDeclarationForType(final TypeElement typeElement) {
    ...
    TypeDeclaration.Kind kind = getKindFromTypeBinding(typeElement);
    // TODO(b/341721484): Even though enums can not have the final modifier, turbine make them final
    // in the header jars.
    boolean isFinal = isFinal(typeElement) && kind != TypeDeclaration.Kind.ENUM;
  ...
  MemberDescriptor getDefaultConstructor(TypeElement clazz) {
